...
export async function findExpedientePorOficina(
  dependencia: string,
  oficina: string,
  tipoDoc: string,
  numDoc: string
  ...
  try {
    ...
    const result = await query(sql, [dependencia, oficina, tipoDoc, numDoc]);

    if (result.length === 0) {
      const result = await query(sql1, [dependencia, oficina, tipoDoc, numDoc]);
    ...
    console.error(
      "Error al obtener el expediente del documento por oficina" + error
    );
    throw new Error(
      "Error al obtener el expediente del documento por oficina."
    );
  }
}

export async function obtenerResumenPorOficina(
  dependencia: string,
  personal: string,
  tipoDoc: string,
  numDoc: string
  ...
  const sql = `SELECT 
    ...
    est_emi.de_est AS estado_destino,
    des.co_dep_des,
...
ORDER BY 
  ...
  try {
    const movimientoBase = await query(sql, [
      dependencia,
      personal,
      tipoDoc,
      numDoc,
    ...
    const exp1 = await findExpedientePorOficina(
      dependencia,
      personal,
      tipoDoc,
      numDoc
    ...
    console.error(
      "Error en el servicio de obtener documentos por oficina: " + error
    );
    //throw new Error("Error en la base de datos por oficina");
    return { expediente: "", documentos: [] };
  }
}
